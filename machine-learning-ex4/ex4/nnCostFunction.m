...
oneHotLabels = zeros(m, nLabels);
for i = 1:nLabels
    oneHotLabels(:,i)=(y == valueLabels(i));
end
X = [ones(m,1) X];
a1 = [ones(m,1) (sigmoid(X*Theta1'))];
z = sigmoid(a1*Theta2');
J = sum(sum(-1*oneHotLabels.*log(z) - (1-oneHotLabels).*log(1-z)))/m + ... 
    lambda*(sum(sum(Theta1(:,2:end).^2)))/(2*m) + lambda*(sum(sum(Theta2(:,2:end).^2)))/(2*m);       
% -------------------------------------------------------------
...
Theta1_grad = Theta1_grad + lambda*temp1;
Theta2_grad = Theta2_grad + lambda*temp2;
